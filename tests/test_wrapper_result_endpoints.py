...
class ResultEndpointTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_result_json_returns_inline_response_from_redis(self):
        rec = _job_record()
        result_text = '[{"title":"Cafe"}]'

        with patch.object(wrapper, "_get_job_from_redis", AsyncMock(return_value=rec)):
            with patch.object(wrapper, "_get_result_from_redis", AsyncMock(return_value=result_text)):
        ...
        self.assertEqual(response.body.decode("utf-8"), result_text)

    async def test_get_result_returns_attachment_response_from_redis(self):
        rec = _job_record()
        result_text = '[{"title":"Cafe"}]'

        with patch.object(wrapper, "_get_job_from_redis", AsyncMock(return_value=rec)):
            with patch.object(wrapper, "_get_result_from_redis", AsyncMock(return_value=result_text)):
                response = await wrapper.get_result(rec.job_id)

        self.assertEqual(
            response.headers["content-disposition"],
            f'attachment; filename="{rec.job_id}.json"',
        )
        self.assertEqual(response.body.decode("utf-8"), result_text)
    ...
    async def test_get_result_json_falls_back_to_disk_and_warms_redis(self):
        rec = _job_record()
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch.object(wrapper, "_get_job_from_redis", AsyncMock(return_value=rec)):
                with patch.object(wrapper, "_get_result_from_redis", AsyncMock(return_value=None)):
                    with patch.object(wrapper, "_result_path_for_read", return_value=result_file):
                        with patch.object(wrapper, "_save_result_to_redis", AsyncMock()) as save_mock:
                            response = await wrapper.get_result_json(rec.job_id)

        save_mock.assert_awaited_once_with(rec.job_id, result_text)
        self.assertEqual(response.body.decode("utf-8"), result_text)

    async def test_get_result_json_raises_410_when_redis_and_file_are_missing(self):
        rec = _job_record()
